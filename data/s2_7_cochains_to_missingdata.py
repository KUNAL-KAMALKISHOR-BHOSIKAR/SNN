...
#     cochains = np.load(f'../../benchmarks/DBHE/base/cochains.npy', allow_pickle=True)
#     simplices = np.load(f'../../benchmarks/DBHE/base/simplices.npy', allow_pickle=True)
...
#     timeit('process')
#     np.save(f'bive_complex/DBHE_base_missing_values.npy', missing_values)
#     np.save(f'bive_complex/DBHE_base_input_damaged.npy', damaged_dataset)
#     np.save(f'bive_complex/DBHE_base_known_values.npy', known_values)
#     timeit('total')
...
def build_missing_values(simplices,percentage_missing_values,max_dim=10):
    ...
    """
    missing_values = [dict() for _ in range(max_dim+1)]
    for i in range(min(max_dim + 1, len(simplices))):
        simp=list(simplices[i].keys())
        l=int(np.ceil((len(simp)/100)*percentage_missing_values))
        simp_copy=np.copy(simp)
        shuffle(simp_copy)
...
def build_damaged_dataset(cochains,missing_values,function=np.median):
    ...
    max_len = max(len(values) if hasattr(values, '__len__') else 1 for d in signal for values in d.values())
    ...
    for d in cochains:
        ...
        for value in d.values():
            if hasattr(value, '__len__'):
                arr.append(list(value) + [0] * (max_len - len(value)))
            else:
                arr.append([value] + [0] * (max_len - 1))
        signal.append(np.array(arr))

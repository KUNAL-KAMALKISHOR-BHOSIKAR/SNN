...
class MySCNN(nn.Module):
    ...
    def forward(self, Ls, Ds, adDs, xs):
        # print(xs)
        assert(len(xs) == 2) # The three degrees are fed together as a list.
...
def main():
    ...
    prefix = sys.argv[1] ##input ./data/s2_3_collaboration_complex

    logdir = sys.argv[2] ##output ./experiments/output
    starting_node=sys.argv[3] # 150250
    percentage_missing_values=sys.argv[4] # 30
    cuda = False

    topdim = 1


    laplacians = np.load('./data/bive_complex/DBHE_base_laplacians.npy',allow_pickle=True)
    boundaries = np.load('./data/bive_complex/DBHE_base_boundaries.npy',allow_pickle=True)

    # print(boundaries)


    Ls =[scnn.scnn.coo2tensor(scnn.chebyshev.normalize(laplacians[i],half_interval=True)) for i in range(topdim+1)] # list containing normalized and converted Laplacian matrices
    Ds=[scnn.scnn.coo2tensor(boundaries[i].transpose()) for i in range(topdim)]
    adDs=[scnn.scnn.coo2tensor(boundaries[i]) for i in range(topdim)]
    ...
    masks_all_deg = np.load('./data/bive_complex/DBHE_base_known_values.npy',allow_pickle=True) ## positive mask= indices that we keep ##1 mask #entries 0 degree
    ...
    signal = np.load('../benchmarks/DBHE/base/cochains.npy',allow_pickle=True)
    ...
    signal = np.load('./data/bive_complex/DBHE_base_input_damaged.npy',allow_pickle=True)
    raw_data=[list(signal[i].values()) for i in range(len(signal))]
    for d in range(0, topdim+1):
